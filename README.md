...
This repo contains an example deployment of microservices to a service mesh
using HashiCorp Waypoint. HashiCorp Consul is the service mesh, and in this
example, there are two Consul clusters peered together. One cluster, named 
`nuka-cola`, runs on AWS EC2s, integrated with a HashiCorp Nomad cluster. The
other, named `sunset-sarsaparilla`, runs on Kubernetes, specifically in an
...
This use case does not cover the set up of the AKS cluster, the AWS EC2s, or
the Consul clusters. However, these configurations are kept in this repo to 
serve as an example of how such resources may be configured, and a reference
for possibly using this demo in the future.
...
of the apps to the two Consul clusters (or rather, to the Nomad and Kubernetes
clusters).

## Pre-requisites

1. Kubernetes cluster & local context connected to this cluster 
   - Local context connected to this cluster 
2. Consul Helm chart deployed to Kubernetes cluster
   - Mesh gateway enabled
   - Additional Kubernetes resources applied to cluster (found in the use case repo under
`sunset-sarsaparilla/kubernetes-config/`)
...
4. An HCP Waypoint server
   ...
   - An OSS Waypoint server will also work, as long as the runners in Nomad
   and Kubernetes can connect to it.
...
![](img/counter-api-nomad-status.png)
...
![](img/counter-ui-inaccessible.png)
...
![](img/countdash-ui-accessible.png)
